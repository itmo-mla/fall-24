...
class ParzenWindowClassifier:
    ...
    def calculate_distance(self, test_point):
        # Вычисляет евклидовы расстояния от тестовой точки до всех точек обучения
    ...
    def _classify(self, test_instance):
        # Шаг 1: Рассчитываем расстояния до всех обучающих точек
        distances = self.calculate_distance(test_instance)

        # Шаг 2: Находим индексы упорядоченных расстояний
        sorted_indices = np.argsort(distances)

        # Шаг 3: Определяем окно ширины по (neighbors+1)-му соседу
        window_width = distances[sorted_indices[self.neighbors]] + 1e-4

        # Шаг 4: Вычисляем веса с помощью ядра для каждой точки обучения
        class_weights = {}
        ...
        # Шаг 5: Возвращаем класс с максимальным весом
        return max(class_weights, key=class_weights.get)

    def predict(self, test_data):
        # Прогнозирование меток для набора тестовых данных
...
class LeaveOneOutValidator:
    ...
    def validate(self, data, labels):
        ...
        # Перебираем значения k от 1 до max_neighbors
        for neighbors_count in range(1, self.max_neighbors + 1):
            ...
            for index in range(len(data)):
                ...
                # Формируем обучающий набор, исключая текущий тестовый элемент
...
def run_experiment():
    ...
    # Шаг 3: Подбор k методом Leave-One-Out
    ...
    # Шаг 4: Обучение модели с найденным k
    ...
    # Шаг 6: Подсчет точности и метрик
    ...
    # Шаг 7: Построение графика ошибки
    ...
    # Отображение минимальной ошибки
    ...
    # Шаг 8: Сравнение с библиотечной реализацией KNN
    ...
    # Шаг 9: Вычисление точности и метрик для библиотечной реализации
